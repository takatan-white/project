...
public class BookRestController {
	...
	public List<BookResource> searchBooks(@Validated BookResourceQuery query){
		...
		String decText = new String(DatatypeConverter.parseHexBinary(query.getName()));
		criteria.setName(decText);
	...
	public List<String> all(@Validated @RequestBody  CompanyParamer resource){
		System.out.println(resource.getCompanyName());
		System.out.println(resource.getCompanyId());
		System.out.println(resource.getCompanyCo()[0] + ":"+ resource.getCompanyCo()[1]);
		return null;
	}
}
